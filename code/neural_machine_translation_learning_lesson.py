"""
neural_machine_translation_learning_lesson.py
==================================================================
...
This script deliberately does two passes:
    ...
    PASS 2 - FIXED TRAINING DATA
        Retrains on a richer dataset that INCLUDES weekday prefixes and
        zero-padded days, then tests the same input again.

Expected corrected translation:
...
def make_dataset(n, include_weekday_styles=False):
    ...
    missing pattern and teaches the model that weekday words are usually
    irrelevant for the target YYYY-MM-DD string.
    """
    data = []
    ...
    for _ in range(n):
        ...
        if style == 0:
            ...
            human = f"{day}/{month}/{year}"                        # 9/10/1993
        elif style == 3:
